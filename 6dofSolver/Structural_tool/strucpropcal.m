...
%--------------- Warhead --------------------------
    ...
    mass_rat_nose = (int_vol / (int_vol_1 + int_vol) ) * (mass_warh - mass_nose - mass_nose_ext);
...
mass_noz = (ext_vol - int_vol )* den_ext;
...
mass_asm = mass_warh_asm + mass_body_asm + mass_noz + mass_fin;
...
cen_tot_asm = ( (cen_nose * (mass_nose + mass_rat_nose)) + (cen_nose_ext_n * (mass_nose_ext + mass_rat_nose_ext)) + (cen_body_n * mass_body_asm) + (cen_noz_n * mass_noz) + (cen_fin_n * mass_fin) ) / (mass_asm);

CG = cen_tot_asm

Ixx = ix_nose + (mass_nose + mass_rat_nose ) * (cen_tot_asm - cen_nose)^2 + ix_nose_ext + (mass_nose_ext + mass_rat_nose_ext) * (cen_tot_asm - cen_nose_ext_n)^2 + ix_body + (mass_body_asm) * (cen_tot_asm - cen_body_n)^2 + ix_noz + (mass_noz) * (cen_tot_asm - cen_noz_n)^2 + ix_fin + (mass_fin) * (cen_tot_asm - cen_fin_n)^2

Iyy = Ixx

Izz = iz_nose + iz_nose_ext + iz_body + iz_noz + iz_fin
